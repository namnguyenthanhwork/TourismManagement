...
public class CMRoleServiceImpl implements CMRoleService {
    ...
    public boolean createRole(RoleEntity roleEntity) {
        ...
        boolean addResult;
        try {
            addResult =  cMRoleRepository.createRole(roleEntity);
        } catch (Exception e) {
            addResult = false;
        }
        return addResult;
    ...
    public boolean updateRole(RoleEntity roleEntity) {
        ...
        slugUtil.setSlug(roleEntity.getRoleName());
        roleEntity.setRoleSlug(slugUtil.getSlug());
        RoleEntity updateRole = cMRoleRepository.getRole(roleEntity.getRoleSlug());
        if(updateRole!=null && updateRole.getRoleId()!= roleEntity.getRoleId())
            return false;
