...
function getFeatureInfo(pageIndex = null) {
    let path = "/TourismManagement/quan-tri-vien/dac-diem-ngay-khoi-hanh/thong-tin"
    if (pageIndex != null)
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                <tr>
                    <td class="text-center">
                        <a href="/TourismManagement/quan-tri-vien/dac-diem-ngay-khoi-hanh/${data[i]['feaSlug']}" 
                        class="badge badge-success text-capitalize">Chỉnh sửa</a>
                        <a href="javascript:;" class="badge badge-danger text-capitalize"
                           onclick="deleteFeature('${data[i]['feaSlug']}')">Xoá</a>
                    </td>
                    ...
                    <td class="align-middle text-center">
                        <span class="text-secondary text-xs font-weight-bold">${data[i]['feaSlug']}</span>
                    </td>

                </tr>     
                `
            }
            document.getElementById('featureInfo').innerHTML = rows
        })

}

function deleteFeature(feaSlug) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`/TourismManagement/quan-tri-vien/dac-diem-ngay-khoi-hanh/${feaSlug}`, {
                method: 'delete'
            }).then(res => {
                return res.status
            }).then(data => {
                ...
                currentPageIndex=1
                getPageAmount()
                getFeatureInfo(currentPageIndex)
            })
        }
    })
}
...
function getPageAmount() {
    fetch('/TourismManagement/quan-tri-vien/dac-diem-ngay-khoi-hanh/so-trang')
